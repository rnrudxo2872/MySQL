module.exports = { //refactoring
  ...
  serch: function (TF) {
    ...
    <form action="/serch" method="get">
      <input type="hidden" name="searchType" value="title+body">
      <input type="text" name="title" placeholder="제목+내용 검색" style="height:22px; width: 100px;
      ...
      <input type="submit" value = "검색" style="width: 40px; height: 30px;color: mintcream;
                                                 ...
                                                 position: relative; left: 67px; bottom: 28.5px">
  </form>
  `
    }
    else if(TF === true){
    ...
    <form action="/serch" method="get">
      ...
      <option value="title+body" selected>제목과 내용</option>
      <option value="title">제목</option>
      <option value="body">내용</option>
   </select>
      <input type="text" name="title" placeholder="검색" style="height:27px; width: 400px;">
      
      <input type="submit" value = "검색" style="width: 40px; height: 35px;color: mintcream;
                                                 background-color: olive;
                                                 font-size: 9pt;
                                                 border-radius: 5%;opacity: 0.5;">
  </form>
  ...
  serch_result(topics) {
    ...
    while (i < topics.length) {
      tag += `
      <tr>
        <th scope="row"><a href="/topic/${topics[i].topic_id}">${topics[i].title}</a></th>
        <td>${topics[i].author}</td>
        <td>${topics[i].created}</td>
    </tr>`
      i++;
    }
